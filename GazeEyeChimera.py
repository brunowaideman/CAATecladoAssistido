...
class Detector:
    ...
    def get_frame(self, frame):
        ...
        if len(dets) == 1:
            ...
            DetectorCapture = False
            self.l_eye_img = np.zeros((64, 64, 3))
            self.r_eye_img = np.zeros((64, 64, 3))
            self.face_img = np.zeros((64, 64, 3))
            self.face_align_img = np.zeros((64, 64, 3))
            self.head_pos = np.ones((64, 64))
            self.head_angle = 0.0
        return (
            DetectorCapture,
            self.l_eye_img,
...
if __name__ == "__main__":
    detector = Detector(
        output_size=256, show_stream=True, show_output=True, show_markers=False
    )
